	...
	<div id="c_main">
		...
		<div class="page_section">
			...
			<c:if test="${member.authority == 1}">
				<div id="nWritebtn">
					<button type="button" id="nWrite" name="nWrite"
						onclick="location.href='./noticeWrite.jsp'">글쓰기</button>
				</div>
			</c:if>
			...
			<div class="pagediv">
				<c:if test="${startPage != 1}">
					<a href="noticeList.do?pageNum=${prev}">이전</a>
				</c:if>
				<c:if test="${startPage != endPage}">
					<c:forEach varStatus="s" begin="${startPage}" end="${endPage}"
						step="1">

						<a href="noticeList.do?pageNum=${s.current}">${s.current}</a>
						<!--변경 : href 경로 -->
					</c:forEach>
				</c:if>
				<c:if test="${next > pageCount}">
					<a href="noticeList.do?pageNum=${next}">다음</a>
				</c:if>
			</div>
		</div>
		<table class="xans-board-search xans-board-search2">
			<tbody>
				<tr>
					<td>
						<button type="button" id="writebtn" name="writebtn" onclick="location.href='noticeWrite.jsp'">글쓰기</button>
					</td>
					<td>
						<div class="search_bt">
							<form name="noticeSearch" action="noticeSearch.do">
								<button type="submit" id="nSearchbtn" name="nSearchbtn"
