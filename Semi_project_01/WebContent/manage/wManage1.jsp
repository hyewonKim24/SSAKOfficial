							...
							<c:if test="${not empty wdlist}">
								...
								<c:forEach items="${wdlist}" var="wd" varStatus="s">
									...
									<tr class="notice1">
										...
										<td class="not1_wdrid"><a
											href="wreqDetail?mid=${wd.mid }">${wd.mid }</a></td>
					...
					<div class="wdrManage_pagination">
						<div class="wdrManage_pagination_warp">
							<c:if test="${startPage != 1}">
								<a href="wreqList?pageNum=${prev}"><</a>
							</c:if>
							<c:if test="${startPage != endPage}">
								<c:forEach varStatus="s" begin="${startPage}" end="${endPage}"
									step="1">
									<a href="wreqList?pageNum=${s.current}">${s.current}</a>
									<!--변경 : href 경로 -->
								</c:forEach>
							</c:if>
							<c:if test="${next < pageCount}">
								<a href="wreqList?pageNum=${next}">></a>
							</c:if>
						</div>
					</div>
				</form>
