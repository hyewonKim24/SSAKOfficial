 ...
 */
@WebServlet("/bookNew.do")
public class bookNewCtrl extends HttpServlet implements Servlet {
...
//		}
		...
		try {			
			...
			List<BookVO> list = bsv.newList(startRnum , endRnum);  /*변경 : 메소드*/
			...
			request.setAttribute("newList", list);	/* 변경 : el태그 - jsp이랑 맞추기  */
			System.out.println(list.size()+ ", " + startPage + ", "+ endPage);
			RequestDispatcher disp = request.getRequestDispatcher("./book/bookNew.jsp"); /*변경 : 경로*/
			disp.forward(request, response);
